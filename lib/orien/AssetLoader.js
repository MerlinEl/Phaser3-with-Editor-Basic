...
class AssetLoader {
	...
	*/
	...
	static loadPipeline(scene, filterName){
// Ctrace("has:{0}", scene.renderer.pipelines.has(filterName))
// Ctrace("get:{0}", scene.renderer.pipelines.get(filterName))
// console.dir(scene)
		if (scene.renderer.pipelines.has(filterName)) return;
		scene.load.script(filterName, "assets/filters/" + filterName + ".js");
	}
	/**

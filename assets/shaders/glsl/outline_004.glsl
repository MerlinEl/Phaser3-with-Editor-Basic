...
void main(){
    ...
    float r = min(u_resolution.x*0.2, u_resolution.y*0.2);
	vec2 uv = gl_FragCoord.xy / r; //v_texcoord
    uv.x /= 8.0;
    uv.y = 1.0 - uv.y;
    vec3 c = texture2D(u_tex_0, uv).rgb;
    float a = texture2D(u_tex_0, uv).a;
    ...
    for (int x = -h_md; x != h_md; ++x){
        for (int y = -h_md; y != h_md; ++y){
            ...
            float o_a = texture2D(u_tex_0, s).a;
